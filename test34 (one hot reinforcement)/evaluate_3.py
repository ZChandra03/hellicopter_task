#!/usr/bin/env python3
"""
UPDATED evaluate_batch3.py
==========================
Batch‑evaluate **24 GRU models** (first three seeds of each of the eight
training types) against the normative Bayesian observer, under the *new*
folder layout:

models/
├── inf_norm/   ┬── seed_0/ checkpoint_best.pt
│               ├── seed_1/ …
│               └── seed_2/ …
├── inf_truth/  ┬── seed_0/
│               └── …
...
Test data live in:
    variants/test/unsorted/test_unsorted_*.csv  (900 trials each)

For each network we print a concise summary plus the **Pearson r** between its
hazard‑error vector and the normative model’s hazard‑error vector.
Pair‑wise correlations *between* networks have been dropped per request.
"""
...
from rnn_models import GRUModel           # ← all eight training specs use GRU
...
MODEL_TYPES  = [  # evaluate in a stable order
    "inf_norm", "inf_truth",
    "unin_norm", "unin_truth",
    "mis_norm", "mis_truth",
    "uns_norm", "uns_truth",
]
SEEDS        = [0, 1, 2]                  # first three seeds → 24 nets
...
def evaluate_network(type_key: str, seed: int, err_norm: np.ndarray) -> None:
    ...
    if errs.std() < 1e-12 or err_norm.std() < 1e-12:
        r = float('nan')
    else:
        r = float(np.corrcoef(errs, err_norm)[0, 1])

    tag_disp = f"{type_key}_s{seed}".upper()
    print(f"{tag_disp:<20}: report {rep:6.3%} | hazard ±0.10 {haz:6.3%} | hazard Hi/Lo {hilo:6.3%} | loss {loss:.4f} | r(NORM) = {r:+.4f}")


# ─────────────────────────── entry‑point ────────────────────────────────

def main():
    # Normative baseline first
    print("========= Bayesian Normative Baseline =========")
    rep_n, haz_n, hilo_n, loss_n, err_norm = evaluate_norm()
    print(f"NORM: report {rep_n:6.3%} | hazard ±0.10 {haz_n:6.3%} | hazard Hi/Lo {hilo_n:6.3%} | loss {loss_n:.4f}\n")
