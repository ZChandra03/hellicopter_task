...
This version:
  ...
  - trains the report head on sign(last evidence), matching the
    last-evidence report heuristic
...
TRAIN_HEADS_TO_RUN = ["both"]  # any of "rep", "haz", or "both"
SEED_START = 0
N_SEEDS = 10
...
def get_default_hp(loss_type: str = "reinforce", train_heads: str = "both") -> dict:
    ...
    return {
        ...
        "report_target": "sign_last_evidence",
        "loss_type": loss_type,
...
class HelicopterDataset(Dataset):
    ...
    Targets:
      - report -> sign(last evidence) (-1/+1 -> 0/1)
    ...
    def __init__(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            yr.append(torch.tensor([last_evidence_report_target(evid)], dtype=torch.float32))
...
def last_evidence_report_target(evidence: list[float]) -> float:
    """Return the 0/1 report target implied by the final evidence sign."""
    if len(evidence) == 0:
        raise ValueError("Evidence sequence cannot be empty")

    final_evidence = float(evidence[-1])
    if final_evidence == 0.0:
        raise ValueError("Final evidence has no sign")

    return 1.0 if final_evidence > 0.0 else 0.0
