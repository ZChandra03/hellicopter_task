#!/usr/bin/env python3
"""
UPDATED evaluate_3_max.py (2025‑07‑23)
======================================
* Compute per‑head cross‑entropy losses (rep / haz) alongside the weighted
  total loss used previously.
* Print rep_loss, haz_loss and total_loss for the Bayesian normative
  baseline and for every trained network.
* No other evaluation logic was changed.
"""
...
SEEDS         = [0]                 # evaluate first three seeds → 24 nets
MAX_TEST_CSVS = 10                  # cap on unsorted‑test CSVs to load
HEAD_WT       = 0.5                 # weight applied to hazard‑loss in total
...
def _clip(p: float) -> float:
    """Avoid log(0) in loss calculations."""
...
def _load_model(type_key: str, seed: int) -> torch.nn.Module:
    ckpt = os.path.join(MODELS_DIR, type_key, f"seed_{seed}", "checkpoint_ep500.pt")
...
# ───────────────────────── per‑CSV evaluation ───────────────────────────
def _cross_entropy(p: float, y: float) -> float:
    """Binary cross‑entropy for scalar probability *p* and label *y* (0/1)."""
...
def _evaluate_csv(csv_path: str, model: torch.nn.Module | None) -> List[Dict]:
    ...
    with torch.no_grad():
        for _, row in df.iterrows():
            ...
            y_rep     = 0.5 * (rep_true + 1)          # 0 or 1 label
            ...
            rep_loss_norm = _cross_entropy(p_state2_norm, y_rep)
            haz_loss_norm = _cross_entropy(p_haz_norm, haz_true)
            tot_loss_norm = rep_loss_norm + HEAD_WT * haz_loss_norm
            ...
            rec = {
                ...
                # losses --------------------------------------------------------
                "rep_loss_norm": rep_loss_norm,
                "haz_loss_norm": haz_loss_norm,
                "loss_norm"    : tot_loss_norm,
                # error for Pearson r -------------------------------------------
            ...
            if model is not None:
                ...
                rep_loss_net = _cross_entropy(p_state2_net, y_rep)
                haz_loss_net = _cross_entropy(p_haz_net,  haz_true)
                tot_loss_net = rep_loss_net + HEAD_WT * haz_loss_net

                rec.update({
                    "rep_corr_net": int(rep_pred == rep_true),
                    "haz_acc_net" : int(abs(p_haz_net - haz_true) <= 0.10),
                    "haz_hilo_net": int((p_haz_net > .5) == (haz_true > .5)),
                    "rep_loss_net": rep_loss_net,
                    "haz_loss_net": haz_loss_net,
                    "loss_net"    : tot_loss_net,
...
def _aggregate_metrics(recs: List[Dict], use_net: bool) -> Tuple[float, float, float]:
    """Return (rep_acc, haz_acc_±0.10, haz_hi/lo)."""
    df  = pd.DataFrame(recs)
    cols = ("rep_corr_net", "haz_acc_net", "haz_hilo_net") if use_net else (
           "rep_corr_norm", "haz_acc_norm", "haz_hilo_norm")
    return tuple(df[c].mean() for c in cols)  # type: ignore[return-value]


def _aggregate_losses(recs: List[Dict], use_net: bool) -> Tuple[float, float, float]:
    """Return mean (rep_loss, haz_loss, total_loss)."""
    df = pd.DataFrame(recs)
    if use_net:
        return (
            df["rep_loss_net"].mean(),
            df["haz_loss_net"].mean(),
            df["loss_net"].mean(),
        )
    return (
        df["rep_loss_norm"].mean(),
        df["haz_loss_norm"].mean(),
        df["loss_norm"].mean(),
    )

# ───────────────────────── top‑level evals ──────────────────────────────
def evaluate_norm() -> Tuple[float, float, float, float, float, float, np.ndarray]:
    csvs = _list_test_csvs(MAX_TEST_CSVS)
    recs: List[Dict] = []
    for p in csvs:
        recs.extend(_evaluate_csv(p, model=None))

    rep_acc, haz_acc, haz_hi_lo = _aggregate_metrics(recs, use_net=False)
    rep_loss, haz_loss, tot_loss = _aggregate_losses(recs, use_net=False)
    errs = np.array([r["haz_err_norm"] for r in recs], dtype=float)

    return rep_acc, haz_acc, haz_hi_lo, rep_loss, haz_loss, tot_loss, errs
...
def evaluate_network(type_key: str, seed: int, err_norm: np.ndarray) -> None:
    ...
    rep_acc, haz_acc, haz_hi_lo = _aggregate_metrics(recs, use_net=True)
    rep_loss, haz_loss, tot_loss = _aggregate_losses(recs, use_net=True)
    ...
    print(f"{tag_disp:<20}: "
          f"rep {rep_acc:6.3%} | haz ±0.10 {haz_acc:6.3%} | haz Hi/Lo {haz_hi_lo:6.3%} | "
          f"loss(rep) {rep_loss:.4f} | loss(haz) {haz_loss:.4f} | total {tot_loss:.4f} | "
          f"r(NORM) = {r:+.4f}")

# ───────────────────────── entry‑point ──────────────────────────────────
def main():
    print("========= Bayesian Normative Baseline =========")
    rep_a, haz_a, hilo_a, rep_l, haz_l, tot_l, err_norm = evaluate_norm()
    print(f"NORM: rep {rep_a:6.3%} | haz ±0.10 {haz_a:6.3%} | haz Hi/Lo {hilo_a:6.3%} | "
          f"loss(rep) {rep_l:.4f} | loss(haz) {haz_l:.4f} | total {tot_l:.4f}\n")
