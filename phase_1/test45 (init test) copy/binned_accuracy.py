#!/usr/bin/env python3
# binned_accuracy.py
# Tests all models on variants/beta_1p0 (first 50 files), uses seeds 0..9, initial.pt only.
# Saves: absolute accuracy figs + delta vs kaiming_uniform, with faint per-seed lines and bold means.
# Colors/labels are FIXED to match the legend.
#
...
#   ./initialized_models/<init_type>/seed_<k>/{initial.pt, hp.json}
...
MODELS_DIR = os.path.join(BASE_DIR, "initialized_models")
...
# Pin colors & labels for the 8 initialization types
MODEL_SPECS = [
    ("kaiming_uniform", "Kaiming Uniform",   "tab:blue"),
    ("orthogonal",      "Orthogonal",        "tab:orange"),
    ("xavier_uniform",  "Xavier Uniform",    "tab:green"),
    ("xavier_normal",   "Xavier Normal",     "tab:red"),
    ("kaiming_normal",  "Kaiming Normal",    "tab:purple"),
    ("sparse",          "Sparse",            "tab:brown"),
    ("uniform",         "Uniform Small",     "tab:pink"),
    ("normal",          "Normal Small",      "tab:gray"),
]
...
def load_model(init_type, seed):
    seed_dir = os.path.join(MODELS_DIR, init_type, f"seed_{seed}")
    ckpt = os.path.join(seed_dir, "initial.pt")  # initial.pt for untrained models
...
def main():
    ...
    rep_acc, haz_acc = {}, {}
    for init_type, _, _ in MODEL_SPECS:
        model_path = os.path.join(MODELS_DIR, init_type)
        if not os.path.isdir(model_path):
            print(f"[warn] missing model folder: {init_type} — skipping")
            continue
        seeds_dict_rep, seeds_dict_haz = {}, {}
        any_seed = False
        for s in SEEDS:
            model = load_model(init_type, s)
            if model is None:
                print(f"[skip] {init_type}/seed_{s}: missing initial.pt or hp.json")
                continue
            any_seed = True
        ...
        if any_seed:
            rep_acc[init_type] = seeds_dict_rep
            haz_acc[init_type] = seeds_dict_haz

    # --- absolute accuracy plots (per-seed faint + bold mean) ---
    # Report head
    fig1, ax1 = plt.subplots(figsize=(10, 6))
    for init_type, label, color in MODEL_SPECS:
        if init_type not in rep_acc: continue
        seeds_dict = rep_acc[init_type]
    ...
    ax1.set_title("Untrained Model Accuracy vs hazard — tested on beta_1p0 (first 20 configs), seeds 0–9")
    ax1.set_ylim(0, 1)
    ax1.grid(True, alpha=0.3)
    ax1.legend(frameon=False, ncol=2)
    fig1.tight_layout()
    save_fig(fig1, "report_head_accuracy_by_hazard_untrained")

    # Hazard head
    fig2, ax2 = plt.subplots(figsize=(10, 6))
    for init_type, label, color in MODEL_SPECS:
        if init_type not in haz_acc: continue
        seeds_dict = haz_acc[init_type]
        for arr in seeds_dict.values():
    ...
    ax2.set_title("Untrained Hazard-head vs hazard — tested on beta_1p0 (first 20 configs), seeds 0–9")
    ax2.set_ylim(0, 1)
    ax2.grid(True, alpha=0.3)
    ax2.legend(frameon=False, ncol=2)
    fig2.tight_layout()
    save_fig(fig2, "hazard_head_accuracy_by_hazard_untrained")

    # --- delta vs kaiming_uniform (baseline mean) ---
    baseline_key = "kaiming_uniform"
    if baseline_key not in rep_acc or baseline_key not in haz_acc:
        print("[warn] kaiming_uniform not available; skipping delta plots.")
    else:
        ...
        for init_type, label, color in MODEL_SPECS:
            if init_type == baseline_key or init_type not in rep_acc: continue
            seeds_dict = rep_acc[init_type]
            for arr in seeds_dict.values():
        ...
        ax3.set_ylabel("Δ Report accuracy vs Kaiming Uniform")
        ax3.set_title("Report-head: accuracy difference relative to Kaiming Uniform")
        ax3.grid(True, alpha=0.3)
        ax3.legend(frameon=False, ncol=2)
        fig3.tight_layout()
        save_fig(fig3, "report_head_accuracy_delta_vs_kaiming")

        # Hazard deltas
        fig4, ax4 = plt.subplots(figsize=(10, 6))
        for init_type, label, color in MODEL_SPECS:
            if init_type == baseline_key or init_type not in haz_acc: continue
            seeds_dict = haz_acc[init_type]
            for arr in seeds_dict.values():
        ...
        ax4.set_ylabel("Δ Hazard-head accuracy vs Kaiming Uniform")
        ax4.set_title("Hazard-head: accuracy difference relative to Kaiming Uniform")
        ax4.grid(True, alpha=0.3)
        ax4.legend(frameon=False, ncol=2)
        fig4.tight_layout()
        save_fig(fig4, "hazard_head_accuracy_delta_vs_kaiming")

    # Optional CSV of mean lines
    rows = []
    for init_type, label, _ in MODEL_SPECS:
        if init_type not in rep_acc: continue
        mean_rep = np.nanmean(np.vstack(list(rep_acc[init_type].values())), axis=0)
        mean_haz = np.nanmean(np.vstack(list(haz_acc[init_type].values())), axis=0)
        for i, c in enumerate(BIN_CENTERS):
            rows.append({"init_type": init_type, "label": label,
                         "bin_center": float(c),
                         "report_acc_mean": float(mean_rep[i]),
                         "hazard_acc_mean": float(mean_haz[i])})
    pd.DataFrame(rows).to_csv(os.path.join(OUT_DIR, "binned_acc_untrained_means.csv"), index=False)
    print("Saved means CSV -> figures/binned_acc_untrained_means.csv")
