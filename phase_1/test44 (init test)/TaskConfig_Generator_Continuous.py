...
==================================
Generate *continuous-hazard* configs for a flat Beta(1, 1) prior and
two different evidence noise levels σ ∈ {1, 2}.
...
1. sets ``params['hazard_beta'] = (1.0, 1.0)`` (uniform prior);
2. sets ``params['testSigmas'] = [sigma]``;
3. writes ``TRAIN_VARIANTS`` train CSVs -- and, if requested,
...
params: Dict[str, Any] = {
    ...
    # Beta prior (fixed to flat Beta(1,1))
    'hazard_beta'   : (1.0, 1.0),
    ...
    # I/O
    'train_variants': 500,      # how many train CSVs per sigma
    'test_variants' : 50,       # how many   test CSVs per sigma
...
def genBlockHazards(p: Dict[str, Any]):
    """Draw one hazard value per trial from the Beta(1,1) prior (no snapping)."""
    nT = int(p['nTrials'])
    a, b = p['hazard_beta']
...
if __name__ == '__main__':
    ...
    for sigma in SIGMA_LIST:
        ...
        print(f"[TCG] sigma={sigma}  →  {params['saveDir']}  (train={params['train_variants']})")
