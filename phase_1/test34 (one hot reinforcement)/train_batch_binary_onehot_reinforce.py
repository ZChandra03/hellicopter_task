...
def reinforce_loss(o_rep_last: torch.Tensor, o_haz: torch.Tensor,
                   ...
                   state: _PGState,
                   entropy_coef: float = 0.01) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Policy-gradient (REINFORCE) estimator for the discrete reward:
      R = 0.5 * (1[a_rep == y_rep] + 1[a_haz == y_haz])

    Returns
    -------
    loss : torch.Tensor scalar (to minimize)
    reward_mean : torch.Tensor scalar detached (mean reward in [0,1])

    Notes
    -----
    - We sample Bernoulli actions from the heads' logits.
    - We use a moving-average baseline to reduce variance.
    - We add a small entropy bonus to encourage exploration.
    """
    # shapes to (B,)
    ...
    with torch.no_grad():
        batch_mean = R.mean()
        state.baseline = 0.9 * state.baseline + 0.1 * batch_mean.item()

    adv = R - state.baseline  # (B,)
    ...
    # encourage exploration slightly
    entropy = (m_r.entropy() + m_h.entropy()).mean()
    loss = pg_loss - entropy_coef * entropy

    return loss, batch_mean.detach()
