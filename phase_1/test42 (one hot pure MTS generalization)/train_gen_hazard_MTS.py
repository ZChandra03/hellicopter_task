...
def get_default_hp() -> dict:
    return {
        ...
        "epochs_per_csv": 4,         # several passes per CSV
        "max_csv": 500,              # take first max_csv from each folder
    }
...
def train_group(
    ...
    # directories ----------------------------------------------------------
    model_dir = os.path.join(BASE_DIR, "models_MTS", group_key, f"seed_{seed}")
