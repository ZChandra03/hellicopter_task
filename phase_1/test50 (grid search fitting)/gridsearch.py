#!/usr/bin/env python3
# gridsearch_simple.py
# Grid search to find best (sigma, bias) that makes Bayesian model match trained GRU
# Follows binned_accuracy.py structure closely
...
HP_PATH = os.path.join(BASE_DIR, "trained_gru", "hp.json")
CHECKPOINT_PATH = os.path.join(BASE_DIR, "trained_gru", "checkpoint_best.pt")

with open(HP_PATH, 'r') as f:
    hp_config = json.load(f)

DATASET_NAME = hp_config.get('sigma_key', 'sigma_1')
N_TEST_CFGS = 50
...
def main():
    print("="*70)
    print("GRID SEARCH: Fitting Bayesian Observer to Trained GRU")
    print("="*70)
    ...
    # Save results
    csv_path = os.path.join(OUT_DIR, f"grid_search_results_{DATASET_NAME}.csv")
    results_df.to_csv(csv_path, index=False)
    print(f"\n✓ Saved overall results to {csv_path}")
    
    # Save binned accuracy for all grid points
    binned_csv_path = os.path.join(OUT_DIR, f"grid_search_binned_{DATASET_NAME}.csv")
    ...
    plot_heatmaps(results_df, DATASET_NAME)
    plot_1d_slices(results_df, DATASET_NAME)
    plot_binned_comparison(binned_df, best_combined['sigma'], 
                          best_combined['bias'], DATASET_NAME)
    plot_all_bayesian_fits(binned_df, results_df, DATASET_NAME)
    ...
    print(f"3. {OUT_DIR}/grid_search_heatmap_{DATASET_NAME}.png")
    print(f"4. {OUT_DIR}/grid_search_slices_{DATASET_NAME}.png")
    print(f"5. {OUT_DIR}/gru_vs_bayesian_binned_{DATASET_NAME}.png")
    print(f"6. {OUT_DIR}/gru_vs_bayesian_variants_{DATASET_NAME}.png")
