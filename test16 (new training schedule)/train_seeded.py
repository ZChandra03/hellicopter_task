...
# `TaskConfig_Generator.makeBlockTrials` to build a brand‑new set of trials,
# train on it once, then discard it. Training stops when either `max_epochs`
# is reached or the running loss drops below `target_loss`.
# ----------------------------------------------------------------------------
import os
import json
import time
...
class HelicopterDataset(Dataset):
    ...
    def __init__(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            haz_val = float(row["trueHazard"])            # probability label
...
def generate_trials() -> pd.DataFrame:
    ...
    trials = TCG.makeBlockTrials(params)  # returns list[dict]
