...
def train_model(model_cls: Type[nn.Module], tag: str, use_norm: bool, seed: int = 0):
    ...
    for epoch in range(hp["max_epochs"]):
        ...
        if best_loss < hp["target_loss"]:
            print(f"{tag} | early stop at epoch {epoch} (loss {best_loss:.4e})")
            break
    ...
    print(f"{tag} finished in {time.time() - t_start:.1f}s | best {best_loss:.4e}")
...
def main():
    ...
    for cls, base_tag, use_norm in MODEL_SPECS:
        for seed in range(30):                     # train 30 seeds
            tag = f"{base_tag}_s{seed}"            # e.g. gru_truth_s3
            train_model(cls, tag, use_norm, seed=seed)

    print("All models done — 30 seeds each, fresh trials every epoch.")
