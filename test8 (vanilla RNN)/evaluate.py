"""evaluate.py – compare RNN vs Normative model, plot error histograms,
...
# ── CHANGED: import the vanilla-RNN model ------------------------------------
...
MODEL_PATH   = os.path.join(BASE_DIR, "models", "rnn_trained", "checkpoint.pt")  # ← CHANGED
...
def load_model(hp: Dict[str, int]) -> RNNModel:                       # ← CHANGED
    m = RNNModel(hp).to(DEVICE)                                       # ← CHANGED
    m.load_state_dict(torch.load(MODEL_PATH, map_location=DEVICE))
    m.eval()
    return m

# ───────────────────────── evaluation
def evaluate_variant(model: RNNModel, csv_path: str) -> List[Dict]:    # ← CHANGED
    ...
    with torch.no_grad():
        for _, row in df.iterrows():
            ...
            prob_state2_rnn = torch.sigmoid(loc_logit_last).item()     # P(state = 2)
            haz_pred_prob   = torch.sigmoid(haz_logits)[0, 0].item()

            loc_pred       =  1 if loc_logit_last.item() > 0 else -1
            rep_corr_rnn   = int(loc_pred == rep_true)

            haz_err_rnn    = haz_pred_prob - haz_true
            haz_acc_rnn    = int(abs(haz_err_rnn) <= 0.10)
            haz_hilo_rnn   = int((haz_pred_prob > 0.5) == (haz_true > 0.5))
            ...
            recs.append({
                "sigma"            : sigma,
                "hazard"           : haz_true,
                "rep_corr_rnn"     : rep_corr_rnn,
                "haz_acc_rnn"      : haz_acc_rnn,
                "haz_hilo_rnn"     : haz_hilo_rnn,
                "haz_err_rnn"      : haz_err_rnn,
...
def aggregate(records: List[Dict]):
    ...
    grouped = (
        df.groupby(["sigma", "hazard"])
          .agg(N                = ("rep_corr_rnn", "size"),
               report_acc_rnn   = ("rep_corr_rnn",  "mean"),
               hazard_acc_rnn   = ("haz_acc_rnn",   "mean"),
               hazard_hilo_rnn  = ("haz_hilo_rnn",  "mean"),
...
def main(max_variants: int = 40):
    ...
    # ---- print tables ------------------------------------------------------
    print("\nPer-condition performance (RNN vs Normative)")
    print("=" * 100)
    ...
    overall = {
        "RNN" : {
            "report_acc": np.average(table["report_acc_rnn"],  weights=weights),
            "hazard_acc": np.average(table["hazard_acc_rnn"],  weights=weights),
            "hazard_hilo":np.average(table["hazard_hilo_rnn"], weights=weights),
        },
        "Norm": {
    ...
    plt.hist(raw_df["haz_err_rnn"],  bins=bins, alpha=0.6,
             label="RNN", edgecolor="black")
