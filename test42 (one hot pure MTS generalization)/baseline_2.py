...
MODELS_DIR  = os.path.join(HERE, "models_MTS")
...
MODEL_TYPES: List[str] = sorted(
    ...
    if d.startswith("hz_") and os.path.isdir(os.path.join(MODELS_DIR, d))
)
if not MODEL_TYPES:
    raise RuntimeError(f"No hz_* folders found under {MODELS_DIR}")
