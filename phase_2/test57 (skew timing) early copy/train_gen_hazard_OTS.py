...
def validate_run_config() -> None:
    ...
    if not isinstance(TRAIN_CASES_TO_RUN, (list, tuple)) or len(TRAIN_CASES_TO_RUN) == 0:
        raise ValueError("TRAIN_CASES_TO_RUN must be a non-empty list or tuple")
    invalid_cases = [case for case in TRAIN_CASES_TO_RUN if case not in {1, 2, 3, 4}]
    if invalid_cases:
        raise ValueError(f"TRAIN_CASES_TO_RUN contains invalid cases: {invalid_cases}")
...
def _filter_df_for_epoch_case(df: pd.DataFrame, epoch_idx: int, train_case: int) -> pd.DataFrame:
    if epoch_idx >= 2:
        return df.reset_index(drop=True)

    hz = pd.to_numeric(df["trueHazard"], errors="coerce")

    if train_case == 1:
        mask = hz > 0.5 if epoch_idx == 0 else hz < 0.5
    elif train_case == 2:
        mask = hz < 0.5 if epoch_idx == 0 else hz > 0.5
    elif train_case == 3:
        mask = hz.between(0.25, 0.75, inclusive="both") if epoch_idx == 0 else ~hz.between(0.25, 0.75, inclusive="both")
    elif train_case == 4:
        mask = ~hz.between(0.25, 0.75, inclusive="both") if epoch_idx == 0 else hz.between(0.25, 0.75, inclusive="both")
    else:
        raise ValueError(f"Unsupported train_case: {train_case!r}")

    return df.loc[mask].reset_index(drop=True)


def _case_epoch_label(epoch_idx: int, train_case: int) -> str:
    if epoch_idx >= 2:
        return "all_trials"

    if train_case == 1:
        return "hazard_gt_0p5" if epoch_idx == 0 else "hazard_lt_0p5"
    if train_case == 2:
        return "hazard_lt_0p5" if epoch_idx == 0 else "hazard_gt_0p5"
    if train_case == 3:
        return "hazard_in_0p25_0p75" if epoch_idx == 0 else "hazard_out_0p25_0p75"
    if train_case == 4:
        return "hazard_out_0p25_0p75" if epoch_idx == 0 else "hazard_in_0p25_0p75"
    raise ValueError(f"Unsupported train_case: {train_case!r}")


def _prepare_epoch_train_dfs(train_csvs: List[str], epoch_idx: int, train_case: int) -> List[pd.DataFrame]:
    filtered_dfs: List[pd.DataFrame] = []
    for path in train_csvs:
        df = pd.read_csv(path)
        filtered_df = _filter_df_for_epoch_case(df, epoch_idx, train_case)
        if len(filtered_df) > 0:
            filtered_dfs.append(filtered_df)
    return filtered_dfs


def _count_batches_for_epoch(train_dfs: List[pd.DataFrame], batch_size: int) -> int:
    return sum(len(df) // batch_size for df in train_dfs)
...
def train_group(
    ...
    model_dir = os.path.join(model_root, f"case_{train_case}", group_key, f"seed_{seed}")
    ...
    print(
        f"case{train_case}|{group_key}|seed{seed} "
    ...
    for epoch in range(n_total_epochs):
        ...
        epoch_train_dfs = _prepare_epoch_train_dfs(train_csvs, epoch, train_case)
        total_batches_per_epoch = _count_batches_for_epoch(epoch_train_dfs, hp["batch_size"])
        if total_batches_per_epoch <= 0:
            raise RuntimeError(
                f"No train batches available for {group_key} in epoch {epoch + 1} under case {train_case}"
            )
        save_marks = _build_epoch_save_marks(total_batches_per_epoch, CHECKPOINTS_PER_EPOCH)

        csv_order = rng.permutation(len(epoch_train_dfs))
        for idx in csv_order:
            df = epoch_train_dfs[idx]
            dl = DataLoader(
        ...
        print(
            f"case{train_case}|{group_key}|seed{seed} "
            f"[{loss_type}/{train_heads}] "
            f"ep {epoch+1:03}/{n_total_epochs} "
            f"mode {_case_epoch_label(epoch, train_case)} "
        ...
        if best < hp["target_loss"]:
            print(
                f"case{train_case}|{group_key}|seed{seed} [{loss_type}/{train_heads}] "
...
def main() -> None:
    ...
    for train_case in TRAIN_CASES_TO_RUN:
        for seed in seeds:
