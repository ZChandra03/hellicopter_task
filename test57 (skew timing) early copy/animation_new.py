...
models_OTS/
    bce_rep/
        sigma_1/
            seed_0/
                checkpoint_ep001_part01.pt
                checkpoint_ep001_part02.pt
                checkpoint_ep001_part03.pt
                checkpoint_ep001_part04.pt
                checkpoint_ep002_part01.pt
                ...
                final.pt
                hp.json
        sigma_2/
...
CKPT_PART_RE = re.compile(r"checkpoint_ep(\d{3})_part(\d{2})\.pt$")
CKPT_EPOCH_RE = re.compile(r"checkpoint_ep(\d{3})\.pt$")
...
def _normalize_case_arg(case_value: str) -> str:
    case_value = str(case_value)
    if CASE_RE.fullmatch(case_value):
        return case_value
    if case_value.isdigit():
        return f"case_{int(case_value)}"
    raise ValueError(f"Unsupported case specifier: {case_value!r}")


def list_cases(run_root: str, cases_cli: List[str]) -> List[str | None]:
    if not os.path.isdir(run_root):
        return []

    case_dirs = sorted(
        d for d in os.listdir(run_root)
        if CASE_RE.fullmatch(d) and os.path.isdir(os.path.join(run_root, d))
    )

    if len(cases_cli) == 1 and cases_cli[0].lower() == "all":
        return case_dirs if case_dirs else [None]

    requested = [_normalize_case_arg(v) for v in cases_cli]
    if case_dirs:
        return [c for c in requested if os.path.isdir(os.path.join(run_root, c))]

    return [None]


def list_groups(run_root: str, groups_cli: List[str], case_key: str | None = None) -> List[str]:
    search_root = os.path.join(run_root, case_key) if case_key is not None else run_root
    ...
    if len(groups_cli) == 1 and groups_cli[0].lower() == "all":
        ...
        return sorted(
            ...
            if SIGMA_RE.fullmatch(d) and os.path.isdir(os.path.join(search_root, d))
        )
    return groups_cli
...
def _format_frame_label(epoch: int, part: int | None) -> str:
    if part is None:
        return f"ep {epoch:03d}"
    return f"ep {epoch:03d} part {part:02d}"


def list_checkpoints(seed_dir: str, include_final: bool) -> List[Tuple[Tuple[int, int, int], str, str]]:
    ckpts: List[Tuple[Tuple[int, int, int], str, str]] = []

    for p in glob.glob(os.path.join(seed_dir, "checkpoint_ep*.pt")):
        name = os.path.basename(p)

        m_part = CKPT_PART_RE.fullmatch(name)
        if m_part:
            epoch = int(m_part.group(1))
            part = int(m_part.group(2))
            sort_key = (epoch, part, 0)
            label = _format_frame_label(epoch, part)
            ckpts.append((sort_key, label, p))
            continue

        m_epoch = CKPT_EPOCH_RE.fullmatch(name)
        if m_epoch:
            epoch = int(m_epoch.group(1))
            sort_key = (epoch, 0, 0)
            label = _format_frame_label(epoch, None)
            ckpts.append((sort_key, label, p))
            continue
    ...
    if include_final:
        ...
        if os.path.exists(final_path):
            ckpts.append(((10**9, 0, 1), "final", final_path))
...
def build_frames(run_root: str, group_key: str, head: str, n_test: int, include_final: bool, case_key: str | None = None):
    group_root = os.path.join(run_root, case_key, group_key) if case_key is not None else os.path.join(run_root, group_key)
    ...
    frame_to_accs: Dict[Tuple[int, int, int], List[Tuple[int, np.ndarray]]] = {}
    frame_labels: Dict[Tuple[int, int, int], str] = {}
    ...
    for s in seed_ids:
        ...
        for frame_key, frame_label, ckpt in ckpts:
            try:
            ...
            frame_to_accs.setdefault(frame_key, []).append((s, acc))
            frame_labels[frame_key] = frame_label

    if not frame_to_accs:
        raise RuntimeError(f"No evaluations produced for {group_root} (head={head})")

    frames_sorted = sorted(frame_to_accs.keys())
    return frames_sorted, frame_labels, frame_to_accs, seed_ids


def plot_hazard_skew(run_key: str,
                     group_key: str,
                     case_key: str | None,
                     frames_sorted: List[Tuple[int, int, int]],
                     frame_labels: Dict[Tuple[int, int, int], str],
                     acc_by_frame: Dict[Tuple[int, int, int], List[Tuple[int, np.ndarray]]],
                     seed_ids: List[int],
    ...
    skew = np.full((n_frames, n_seeds), np.nan)
    s_index = {s: i for i, s in enumerate(seed_ids)}

    for t, frame_key in enumerate(frames_sorted):
        for s, acc in acc_by_frame[frame_key]:
            hi = np.nanmean(acc[high_mask])
            lo = np.nanmean(acc[low_mask])
            skew[t, s_index[s]] = hi - lo

    fig, ax = plt.subplots(figsize=(max(10.5, 0.8 * n_frames), 5.6))
    ...
    ax.set_xticks(xs)
    ax.set_xticklabels([frame_labels[k] for k in frames_sorted], rotation=45, ha="right")
    title_prefix = f"{run_key} | {case_key} | {group_key}" if case_key is not None else f"{run_key} | {group_key}"
...
def animate_group(run_root: str,
    ...
    frames_sorted, frame_labels, acc_by_frame, seed_ids = build_frames(run_root, group_key, head, n_test, include_final, case_key=case_key)
    seed_color = make_seed_palette(seed_ids)

    out_dir = os.path.join(OUT_ROOT, run_key, case_key, group_key) if case_key is not None else os.path.join(OUT_ROOT, run_key, group_key)
    ...
    def update(frame_idx: int):
        frame_key = frames_sorted[frame_idx]
        seed_acc_map = {s: acc for s, acc in acc_by_frame[frame_key]}
        ...
        title_prefix = f"{run_key} | {case_key} | {group_key}" if case_key is not None else f"{run_key} | {group_key}"
        title.set_text(f"{title_prefix} | {head} head | {frame_labels[frame_key]}")
        return list(lines.values()) + [title]
...
def main():
    ...
    ap.add_argument("--cases", nargs="+", default=["all"], help="case numbers or names (1 2 3 4 or case_1 ...), or all")
    ...
    for loss_type in loss_types:
        for train_heads in train_heads_list:
            ...
            cases = list_cases(run_root, args.cases)
            if not cases:
                print(f"[skip] no matching cases found under {run_root}")
                continue

            any_run_found = True

            for case_key in cases:
                groups = list_groups(run_root, args.groups, case_key=case_key)
                if not groups:
                    search_root = os.path.join(run_root, case_key) if case_key is not None else run_root
                    print(f"[skip] no sigma groups found under {search_root}")
                    continue
